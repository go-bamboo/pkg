...
func TestMakeRequest(t *testing.T) {
	...
	var cases = []struct {
		...
		{POST, NewSuperAgent().Post("/")},
		{GET, NewSuperAgent().Get("/")},
		{HEAD, NewSuperAgent().Head("/")},
		{PUT, NewSuperAgent().Put("/")},
		{PATCH, NewSuperAgent().Patch("/")},
		{DELETE, NewSuperAgent().Delete("/")},
		{OPTIONS, NewSuperAgent().Options("/")},
		{"TRACE", NewSuperAgent().CustomMethod("TRACE", "/")}, // valid HTTP 1.1 method, see W3C RFC 2616
	}
	...
	_, err = NewSuperAgent().CustomMethod("", "/").MakeRequest()
	if err == nil {
...
func TestGet(t *testing.T) {
	...
	NewSuperAgent().Get(ts.URL + case1_empty).
		End()

	NewSuperAgent().Get(ts.URL+case2_set_header).
		Set("API-Key", "fookey").
		End()
}
...
func TestRetryGet(t *testing.T) {
	...
	resp, _, errs := NewSuperAgent().Get(ts.URL+case1_empty).
	...
	resp, _, errs = NewSuperAgent().Get(ts.URL+case24_after_3_attempt_return_valid).
...
func TestResetBody(t *testing.T) {
	...
	resp, _, _ := NewSuperAgent().Get(ts.URL).End()
...
func TestPost(t *testing.T) {
	...
	NewSuperAgent().Post(ts.URL + test_post_case1_empty).
		End()

	NewSuperAgent().Post(ts.URL+test_post_case2_set_header).
		Set("API-Key", "fookey").
		End()

	NewSuperAgent().Post(ts.URL + test_post_case3_send_json).
		Send(`{"query1":"test"}`).
		Send(`{"query2":"test"}`).
		End()

	NewSuperAgent().Post(ts.URL + test_post_case4_send_string).
		Send("query1=test").
		Send("query2=test").
		End()

	NewSuperAgent().Post(ts.URL + test_post_case5_integration_send_json_string).
		Send("query1=test").
		Send(`{"query2":"test"}`).
		End()
	...
	*/
	NewSuperAgent().Post(ts.URL + test_post_case6_set_query).
		Query("query1=test").
		Query("query2=test").
		End()
	// TODO:
	...
	NewSuperAgent().Post(ts.URL + test_post_case7_integration_send_json_struct).
		Send(`{"a":"a"}`).
		Send(myStyle).
		End()

	NewSuperAgent().Post(ts.URL + test_post_case8_send_json_with_long_id_number).
		Send(`{"id":123456789, "name":"nemo"}`).
		End()

	NewSuperAgent().Post(ts.URL + test_post_case9_send_json_string_with_long_id_number_as_form_result).
		Type("form").
		Send(`{"id":123456789, "name":"nemo"}`).
		End()
	...
	NewSuperAgent().Post(ts.URL + test_post_case10_send_struct_pointer).
		Send(&payload).
		End()

	NewSuperAgent().Post(ts.URL + test_post_case19_send_struct).
		Send(payload).
		End()

	s1 := "query1=test"
	s2 := "query2=test"
	NewSuperAgent().Post(ts.URL + test_post_case11_send_string_pointer).
		Send(&s1).
		Send(&s2).
		End()

	NewSuperAgent().Post(ts.URL + test_post_case12_send_slice_string).
		Send([]string{"string1", "string2"}).
		End()

	NewSuperAgent().Post(ts.URL + test_post_case13_send_slice_string_pointer).
		Send(&[]string{"string1", "string2"}).
		End()

	i := 42
	NewSuperAgent().Post(ts.URL + test_post_case14_send_int_pointer).
		Send(&i).
		End()

	f := 12.345
	NewSuperAgent().Post(ts.URL + test_post_case15_send_float_pointer).
		Send(&f).
		End()

	b := true
	NewSuperAgent().Post(ts.URL + test_post_case16_send_bool_pointer).
		Send(&b).
		End()
	...
	a[0] = "string1"
	a[1] = "string2"
	NewSuperAgent().Post(ts.URL + test_post_case17_send_string_array).
		Send(a).
		End()

	NewSuperAgent().Post(ts.URL + test_post_case18_send_string_array_pointer).
		Send(&a).
		End()

	aByte := byte('G') // = 71 dec
	NewSuperAgent().Post(ts.URL + test_post_case20_send_byte_char).
		Send(aByte).
		End()

	NewSuperAgent().Post(ts.URL + test_post_case21_send_byte_char_pointer).
		Send(&aByte).
		End()

	iByte := byte(71) // = 'G'
	NewSuperAgent().Post(ts.URL + test_post_case22_send_byte_int).
		Send(iByte).
		End()

	NewSuperAgent().Post(ts.URL + test_post_case22_send_byte_int_pointer).
		Send(&iByte).
		End()

	NewSuperAgent().Post(ts.URL + test_post_case23_send_duplicate_query_params).
		Send("param=1").
		Send("param=2").
	...
	NewSuperAgent().Post(ts.URL + test_post_case24_send_query_and_request_body).
		Query("test=true").
		Send(data24).
		End()
}
...
func TestMultipartRequest(t *testing.T) {
	...
	_, _, errs := NewSuperAgent().Post(ts.URL + case0_send_not_supported_filetype).
		Type("multipart").
		SendFile(42).
		End()
	...
	NewSuperAgent().Post(ts.URL + case1_send_string).
		Type("multipart").
		Send("query1=test").
		Send("query2=test").
		End()

	NewSuperAgent().Post(ts.URL + case2_send_json).
		Type("multipart").
		Send(`{"query1":"test"}`).
		Send(`{"query2":"test"}`).
		End()

	NewSuperAgent().Post(ts.URL + case3_integration_send_json_string).
		Type("multipart").
		Send("query1=test").
		Send(`{"query2":"test"}`).
		End()

	NewSuperAgent().Post(ts.URL + case4_set_query).
		Type("multipart").
		Query("query1=test").
		Query("query2=test").
		Send("foo").
	...
	NewSuperAgent().Post(ts.URL + case5_send_struct).
		Type("multipart").
		Send(testStruct{
			String:      "a string",
			Int:         42,
	...
	NewSuperAgent().Post(ts.URL + case6_send_slice_string).
		Type("multipart").
		Send([]string{"string1", "string2"}).
		End()

	NewSuperAgent().Post(ts.URL+case6_send_slice_string_with_custom_fieldname).
		Type("multipart").
		Set("json_fieldname", "my_custom_data").
		Send([]string{"string1", "string2"}).
		End()

	NewSuperAgent().Post(ts.URL + case7_send_array).
		Type("multipart").
		Send([2]string{"string1", "string2"}).
		End()

	NewSuperAgent().Post(ts.URL + case8_integration_send_json_struct).
		Type("multipart").
		Send(`{"query1":"test"}`).
		Send(heyYou{
			Hey: "hey",
	...
	NewSuperAgent().Post(ts.URL + case9_send_duplicate_query_params).
		Type("multipart").
		Send("param=1").
		Send("param=2").
		Send("param=3&param=4").
	...
	fileByPath := "./LICENSE"
	NewSuperAgent().Post(ts.URL + case10_send_file_by_path).
		Type("multipart").
		SendFile(fileByPath).
		End()

	NewSuperAgent().Post(ts.URL+case10a_send_file_by_path_with_name).
		Type("multipart").
		SendFile(fileByPath, "MY_LICENSE").
		End()

	NewSuperAgent().Post(ts.URL+case10b_send_file_by_path_pointer).
		Type("multipart").
		SendFile(&fileByPath, "MY_LICENSE").
		End()

	NewSuperAgent().Post(ts.URL+case11_send_file_by_path_without_name).
		Type("multipart").
		SendFile(fileByPath, "").
		End()

	NewSuperAgent().Post(ts.URL+case12_send_file_by_path_without_name_but_with_fieldname).
		Type("multipart").
		SendFile(fileByPath, "", "my_fieldname").
		End()

	b, _ := ioutil.ReadFile("./LICENSE")
	NewSuperAgent().Post(ts.URL + case13_send_file_by_content_without_name).
		Type("multipart").
		SendFile(b).
		End()

	NewSuperAgent().Post(ts.URL + case13a_send_file_by_content_without_name_pointer).
		Type("multipart").
		SendFile(&b).
		End()

	NewSuperAgent().Post(ts.URL+case14_send_file_by_content_with_name).
		Type("multipart").
		SendFile(b, "LICENSE").
		End()

	NewSuperAgent().Post(ts.URL+case15_send_file_by_content_without_name_but_with_fieldname).
		Type("multipart").
		SendFile(b, "", "my_fieldname").
		End()

	NewSuperAgent().Post(ts.URL+case16_send_file_by_content_with_name_and_with_fieldname).
		Type("multipart").
		SendFile(b, "MY_LICENSE", "my_fieldname").
		End()

	NewSuperAgent().Post(ts.URL + case17_send_file_multiple_by_path_and_content_without_name).
		Type("multipart").
		SendFile("./LICENSE").
		SendFile(b).
		End()

	NewSuperAgent().Post(ts.URL+case18_send_file_multiple_by_path_and_content_with_name).
		Type("multipart").
		SendFile("./LICENSE").
		SendFile(b, "MY_LICENSE").
		End()

	NewSuperAgent().Post(ts.URL + case19_integration_send_file_and_data).
		Type("multipart").
		SendFile("./LICENSE").
		Send("query1=test").
		End()
	...
	NewSuperAgent().Post(ts.URL + case20_send_file_as_osfile).
		Type("multipart").
		SendFile(osFile).
		End()

	NewSuperAgent().Post(ts.URL+case21_send_file_as_osfile_with_name).
		Type("multipart").
		SendFile(osFile, "MY_LICENSE").
		End()

	NewSuperAgent().Post(ts.URL+case22_send_file_as_osfile_with_name_and_with_fieldname).
		Type("multipart").
		SendFile(osFile, "MY_LICENSE", "my_fieldname").
		End()

	NewSuperAgent().Post(ts.URL+case23_send_file_with_file_as_fieldname).
		Type("multipart").
		SendFile(b, "b.file").
		SendFile(osFile, "", "file").
		End()

	NewSuperAgent().Post(ts.URL+case24_send_file_with_name_with_spaces).
		Type("multipart").
		SendFile(osFile, " LICENSE  ").
		End()

	NewSuperAgent().Post(ts.URL+case25_send_file_with_name_with_spaces_only).
		Type("multipart").
		SendFile(osFile, "   ").
		End()

	NewSuperAgent().Post(ts.URL+case26_send_file_with_fieldname_with_spaces).
		Type("multipart").
		SendFile(osFile, "", " my_fieldname  ").
		End()

	NewSuperAgent().Post(ts.URL+case27_send_file_with_fieldname_with_spaces_only).
		Type("multipart").
		SendFile(osFile, "", "   ").
		End()

	NewSuperAgent().Post(ts.URL+case28_send_file_with_file_as_fieldname_and_skip_file_numbering_true).
		Type("multipart").
...
func TestPatch(t *testing.T) {
	...
	NewSuperAgent().Patch(ts.URL + case1_empty).
		End()

	NewSuperAgent().Patch(ts.URL+case2_set_header).
		Set("API-Key", "fookey").
		End()

	NewSuperAgent().Patch(ts.URL + case3_send_json).
		Send(`{"query1":"test"}`).
		Send(`{"query2":"test"}`).
		End()
}
...
func TestQueryFunc(t *testing.T) {
	...
	NewSuperAgent().Post(ts.URL + case2_send_struct).
		Query(qq).
		End()

	NewSuperAgent().Post(ts.URL + case3_send_string_with_duplicates).
		Query("query1=test1").
		Query("query2=test2").
		Query("param=1").
		Query("param=2").
	...
	NewSuperAgent().Post(ts.URL + case4_send_map).
...
func TestEndBytes(t *testing.T) {
	...
	// Callback.
	{
		resp, bodyBytes, errs := NewSuperAgent().Get(ts.URL).EndBytes(func(resp Response, body []byte, errs []error) {
			if len(errs) > 0 {
	...
	// No callback.
	{
		resp, bodyBytes, errs := NewSuperAgent().Get(ts.URL).EndBytes()
		if len(errs) > 0 {
...
func TestEndStruct(t *testing.T) {
	...
	// Callback.
	{
		resp, bodyBytes, errs := NewSuperAgent().Get(ts.URL).EndStruct(func(resp Response, v interface{}, body []byte, errs []error) {
			if len(errs) > 0 {
				t.Fatalf("Unexpected errors: %s", errs)
			}
			if resp.StatusCode != 200 {
	...
	// No callback.
	{
		resp, bodyBytes, errs := NewSuperAgent().Get(ts.URL).EndStruct(&resStruct)
		if len(errs) > 0 {
			t.Errorf("Unexpected errors: %s", errs)
		}
		if resp.StatusCode != 200 {
...
func TestProxyFunc(t *testing.T) {
	...
	resp, body, _ := NewSuperAgent().Proxy(ts2.URL).Get(ts.URL).End()
	if resp.StatusCode != 200 {
...
func TestProxyFuncPostClone(t *testing.T) {
	...
	baseRequest := NewSuperAgent().Proxy(proxyTs.URL)
	// sending request via Proxy
...
func TestTimeoutFunc(t *testing.T) {
	...
	startTime := time.Now()
	_, _, errs := NewSuperAgent(Timeout(60 *time.Millisecond)).Get("http://www.google.com:81").End()
	...
	request := NewSuperAgent(Timeout(1000 * time.Millisecond))
	startTime = time.Now()
...
func TestCookies(t *testing.T) {
	request := NewSuperAgent(Timeout(60 *time.Second))
...
func TestGetSetCookies(t *testing.T) {
	...
	NewSuperAgent().Get(ts.URL).AddCookies([]*http.Cookie{
...
// or Set methods.
// https://bls/pkg/net/http/issues/164
...
func TestAsCurlCommand(t *testing.T) {
	// var (
	// 	endpoint = "http://bls/pkg/net/http"
	// 	jsonData = `{"here": "is", "some": {"json": ["data"]}}`
	// )

	// logger := log.NewStdLogger(os.Stdout)
	// request := NewSuperAgent(Timeout(10*time.Second)).Put(endpoint).Set("Content-Type", "application/json").Send(jsonData)
...
func TestSetDebugByEnvironmentVar(t *testing.T) {
	...
	NewSuperAgent(WithLogger(logger)).Get(endpoint).End()
