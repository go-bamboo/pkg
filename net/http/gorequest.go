...
type Request *stdhttp.Request
type Response *stdhttp.Response
...
func WithLogger(logger zapcore.Core) Option {
	return func(o *options) {
		o.logger = logger
	}
}

func DisableKeepAlives(enabl bool) Option {
	return func(o *options) {
		o.keepAlives = enabl
	}
}
...
// NewSuperAgent Used to create a new SuperAgent object.
func NewSuperAgent(opts ...Option) (*SuperAgent, error) {
	_options := options{
	...
	var tp stdhttp.RoundTripper = &stdhttp.Transport{
		...
		DisableKeepAlives:     _options.keepAlives,
	}
	c, err := transporthttp.NewClient(context.TODO(),
		transporthttp.WithTimeout(_options.timeout),
		transporthttp.WithTransport(tp),
	)
	if err != nil {
		return nil, err
	}
	...
	// disable keep alives by default, see this issue https://bls/pkg/net/http/issues/75
...
func shallowCopyCookies(old []*stdhttp.Cookie) []*stdhttp.Cookie {
	if old == nil {
		return nil
	}
	newData := make([]*stdhttp.Cookie, len(old))
	for i := range old {
		newData[i] = old[i]
	}
	return newData
...
func (s *SuperAgent) Clone() *SuperAgent {
	clone := &SuperAgent{
		Url:                  s.Url,
		Method:               s.Method,
		Header:               stdhttp.Header(cloneMapArray(s.Header)),
...
func (s *SuperAgent) ClearSuperAgent() {
	...
	s.Cookies = make([]*stdhttp.Cookie, 0)
	s.Errors = nil
}
...
func (s *SuperAgent) Retry(retryerCount int, retryerTime time.Duration, statusCode ...int) *SuperAgent {
	for _, code := range statusCode {
		statusText := stdhttp.StatusText(code)
		if len(statusText) == 0 {
			s.Errors = append(s.Errors, errors.InternalServer("StatusCode:"+strconv.Itoa(code), "doesn't exist in http package"))
		}
	}
...
func (s *SuperAgent) AddCookie(c *stdhttp.Cookie) *SuperAgent {
	s.Cookies = append(s.Cookies, c)
	return s
}

// AddCookies is a convenient method to add multiple cookies
func (s *SuperAgent) AddCookies(cookies []*stdhttp.Cookie) *SuperAgent {
...
func (s *SuperAgent) Type(typeStr string) *SuperAgent {
	if _, ok := Types[typeStr]; ok {
		...
		s.Errors = append(s.Errors, errors.InternalServer("IncorrectType", "Type func: incorrect type \""+typeStr+"\""))
	}
	return s
}
...
func (s *SuperAgent) SendFile(file interface{}, args ...interface{}) *SuperAgent {
	...
	default:
		...
		s.Errors = append(s.Errors, errors.InternalServer("NotSupportType", "SendFile currently only supports either a string (path/to/file), a slice of bytes (file content itself), or a os.File!"))
	}
...
func (s *SuperAgent) MakeRequest() (*stdhttp.Request, error) {
	var (
		req           *stdhttp.Request
	...
	if s.Method == "" {
		return nil, errors.InternalServer("NoMethodSpecified", "No method specified")
	}
	...
	//
	//     https://bls/pkg/net/http/pull/136
	//
	switch s.TargetType {
	case TypeJSON:
	...
	default:
		...
		return nil, errors.InternalServer("ErrTargetType", "TargetType '"+s.TargetType+"' could not be determined")
	}

	if req, err = stdhttp.NewRequest(s.Method, s.Url, contentReader); err != nil {
		return nil, err
	}
